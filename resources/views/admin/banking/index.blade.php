<x-layouts.admin title="Corporate Bank Autopay &amp; Disbursements">
    ...
    <div class="space-y-8">
                ...
                <div class="space-y-2 max-w-2xl">
                    <div class="flex items-center gap-2.5 flex-wrap">
                        ...
                        <span class="px-2.5 py-0.5 rounded-full text-[10px] font-bold bg-emerald-500/20 text-emerald-300 border border-emerald-400/30 inline-flex items-center gap-1.5 backdrop-blur-xs">
                            ...
                            Maybank2e / CIMB BizChannel / DuitNow
                        </span>
                    </div>
                    <p class="text-xs sm:text-sm text-indigo-100/80 leading-relaxed">
                        Generate corporate electronic bulk salary payment files for automated salary disbursement via Maybank2e, CIMB BizChannel, and Interbank DuitNow/IBG.
                    </p>
                </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <x-stat-card 
                title="Maybank2e MAS"
                value="Fixed Width (.txt)"
                change="Multi-Pay MAS Standard"
                changeType="neutral"
                icon="bx-buildings"
                color="amber"
            />
            <x-stat-card 
                title="CIMB BizChannel"
                value="CSV Direct Credit"
                change="Corporate Batch Format"
                changeType="neutral"
                icon="bx-credit-card"
                color="rose"
            />
            <x-stat-card 
                title="DuitNow / IBG"
                value="Universal Batch"
                change="Public Bank, RHB, HLB"
                changeType="positive"
                icon="bx-transfer"
                color="emerald"
            />
        ...
        <!-- Corporate Bank Autopay Formats -->
        <div class="space-y-4">
        ...
        <!-- Autopay Disbursed Batches Audit Trail -->
        <div class="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-xs overflow-hidden">
            <div class="p-4 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between">
                <div class="flex items-center gap-2.5">
                    ...
                    <div>
                        <h2 class="text-sm font-bold text-slate-900 dark:text-white">Bank Autopay Disbursement Audit Trail</h2>
                        <p class="text-[11px] text-slate-400">Chronological history of generated bank bulk payment files</p>
                    </div>
                </div>
                <span class="text-[11px] text-slate-400 font-mono">{{ $bankBatches->total() }} Batches Recorded</span>
            </div>
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="p-10 text-center text-slate-400">
                                    No bank autopay disbursement records found yet.
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
    ...
    <x-modal id="export-confirm-modal" title="Confirm Bank Autopay File Generation" subtitle="Verify batch parameters before creating official electronic banking file" icon="bx-download" size="md">
        <form id="export-confirm-form" method="POST" action="">
            ...
            <div class="space-y-4 text-left text-xs">
                <div class="p-3.5 rounded-xl bg-amber-50 dark:bg-amber-950/40 border border-amber-200 dark:border-amber-800 flex items-start gap-3 text-amber-800 dark:text-amber-200">
                    ...
                    <div>
                        <span class="font-bold block text-xs">Official Bank Payout File</span>
