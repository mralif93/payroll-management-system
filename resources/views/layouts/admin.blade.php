...
<body class="h-full bg-slate-100/70 dark:bg-slate-950 text-slate-800 dark:text-slate-100 antialiased selection:bg-indigo-500 selection:text-white transition-colors duration-300 flex overflow-hidden" style="font-family: 'Plus Jakarta Sans', sans-serif;">
    ...
    <aside id="admin-sidebar" class="fixed lg:static inset-y-0 left-0 -translate-x-full lg:translate-x-0 w-64 bg-white dark:bg-slate-900 border-r border-slate-200 dark:border-slate-800 flex flex-col shrink-0 z-50 transition-transform duration-300 ease-in-out">
        ...
        <nav class="flex-1 px-4 py-6 space-y-1.5 overflow-y-auto text-xs font-semibold">
            ...
            <a href="/admin/leaves" class="flex items-center gap-3 px-3 py-2.5 rounded-xl {{ request()->is('admin/leaves*') ? 'bg-indigo-50 dark:bg-indigo-950/60 text-indigo-700 dark:text-indigo-300 font-bold' : 'text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800 hover:text-slate-900 dark:hover:text-white' }} transition">
                <i class="bx bx-calendar-event text-lg"></i>
                <span>Leave &amp; Attendance</span>
            </a>

            <a href="/admin/payroll" class="flex items-center gap-3 px-3 py-2.5 rounded-xl {{ request()->is('admin/payroll*') ? 'bg-indigo-50 dark:bg-indigo-950/60 text-indigo-700 dark:text-indigo-300 font-bold' : 'text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800 hover:text-slate-900 dark:hover:text-white' }} transition">
    ...
    <div class="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        <!-- Top App Bar Header (Ultra-Modern SaaS Clean Glass Design) -->
        <header class="h-16 bg-white/90 dark:bg-slate-900/90 backdrop-blur-xl border-b border-slate-200/80 dark:border-slate-800/80 flex items-center justify-between px-4 sm:px-6 lg:px-8 shrink-0 transition-all duration-300 sticky top-0 z-30">
            
            <!-- Left: Mobile Toggle, Brand & Current Section Context -->
            <div class="flex items-center gap-3.5 min-w-0">
                <!-- Mobile Navigation Toggle -->
                <button 
                    type="button" 
                    onclick="toggleMobileSidebar()" 
                    class="lg:hidden w-9 h-9 rounded-xl border border-slate-200/80 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/80 text-slate-700 dark:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-700 transition flex items-center justify-center cursor-pointer shrink-0"
                    title="Open Navigation Menu"
                >
                    <i class="bx bx-menu-alt-left text-xl"></i>
                </button>

                <!-- Page Location & Module Hierarchy -->
            ...
            <!-- Right: System Status, Quick Actions & User Flyout -->
            <div class="flex items-center gap-2 sm:gap-3 shrink-0">
                ...
                <div class="relative" id="notifications-menu-container">
                    <button 
                        ...
                        class="relative w-9 h-9 rounded-xl border border-slate-200/80 dark:border-slate-800 bg-slate-50/80 dark:bg-slate-800/60 text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700 transition flex items-center justify-center cursor-pointer shrink-0"
                ...
                <div class="relative" id="user-menu-container">
                    ...
                    >
                        ...
                        <i class="bx bx-chevron-down text-slate-400 group-hover:text-slate-600 dark:group-hover:text-slate-200 transition-transform duration-200 text-base ml-0.5" id="user-menu-chevron"></i>
                    </button>
